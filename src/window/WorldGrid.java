  ...
  for (int y=0; y<Game.iCellV; y++) {
    for (int x=0; x<Game.iCellH; x++) {
      ...
      if (!GameIPDNet.omStratPMO[x][y].bClusterStrat && (MainWindow.iCellSize > 2) ) {
      	...
      	if (bNoPureStrat)						// Bigger frame in no pure strats
...
private int iGetColorCell_Show_Probs (int x, int y) {
	...
	switch (iIndexMax) {		// Colors are the same as in the Window: Freq. per Action 
		case 1:		iColor = 2; break;		// Red (P1)
		case 2:		iColor = 1; break;		// Orange (P2)
		case 3:		iColor = 4; break;		// Blue (P3)
		case 4:		iColor = 6; break;		// Green (P4)
		default:	iColor = 0; break;		// Black (P0)
	}
